module.exports = (node, graph) => {
  ...
  function fillVRep3(acc_motifArr,nIn, vRep, sliced_chains) {
    ...
    // ----------
    let singleDiamond = new DovetailObj(nIn, chains.slice(0, nIn * 2));
    ...
    for (let rep = 1; rep < hRep + 1; rep++) {
      ...
      if (!vOverlap && !hOverlap) {
        let hrDiamond = new DovetailObj(nIn, chains.slice(rep * (nIn * 2), (rep + 1) * (nIn * 2)));
        ...
        if (vRep > 0) {
          let fillDiamond = new DovetailObj(nFill, chains.slice(nIn + (rep-1) * (nIn * 2), nIn + rep * (nIn * 2)));
        ...
        if (vRep > 0) {
          let fillDiamond = new DovetailObj(nFill, chains.slice(nFill + (rep-1) * (nIn * 2), nFill + rep * (nIn * 2)));
          let fillMotifArr = fillDiamond.motifArr;
