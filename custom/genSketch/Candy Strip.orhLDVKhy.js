module.exports = (node, graph) => {
  ...
  const numForwardIn = node.in("#forward", 2, {precision: 0, min: 2, max: 20});
  const numBackwardIn = node.in("#backward", 0, {precision: 0, min: 0, max: 19});
  ...
  function produceVRep(acc_motifArr, numF, numB, vRep, sliced_chains) {
    ...
    let numB = numBackwardIn.value+1;
