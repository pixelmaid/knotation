module.exports = (node, graph) => {
  ...
  function mirrorKnotType(knotType) {
    ...
    return mirrorPairs.get(knotType);
  }
    ...
    if (activate) {
      ...
      if (symmetric) {
        let y = totalChainNum - 1 - colNum -1;
        for (let x = 0; x < motifArr.length; x++) {
          if (motifArr[x][y] != 0 && y != colNum){
            motifArr[x][y].setKnotType(mirrorKnotType(newKnotType));
          }
        }
      }
